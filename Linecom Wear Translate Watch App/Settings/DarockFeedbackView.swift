    ...
    var body: some View {
        List {
            Section {
                ...
                //NavigationLink(destination: { SUICChatSupportView(projName: "LWT") }, label: {
                                    ...
                                    NavigationLink(destination: {
                                        List {
                                            NavigationLink(destination: {
                                                ScrollView {
                                                    HStack {
                                                        Text("AppVersion: v\(Bundle.main.infoDictionary?["CFBundleShortVersionString"] as! String) Build \(Bundle.main.infoDictionary?["CFBundleVersion"] as! String)")
                                                            .font(.system(size: 13))
...
UDID: \(WKInterfaceDevice.current().identifierForVendor?.uuidString ?? "None")
""")
                                                            .font(.system(size: 13))
                                                            .multilineTextAlignment(.leading)
                                                        Spacer()
                                ...
                                HStack {
                                    Text("LinecomWearTranslate Appdiagnose")
                                    Spacer()
                    ...
                    Section {
                        Button(action: {
                            ...
                            DarockKit.Network.shared
                                .requestString("https://fapi.darock.top:65535/feedback/submit/anony/LWT/\(msgToSend.base64Encoded().replacingOccurrences(of: "/", with: "{slash}"))")
            ...
            .alert("移除诊断信息", isPresented: $isRemoveDiagAlertPresented, actions: {
                ...
                Text("移除诊断信息会限制 Linecom LLC 理解并解决问题的能力。")
            })
                ...
                TextField("回复信息", text: $replyInput)
                    .onSubmit {
                        ...
                        if replyInput != "" {
                            ...
                            DarockKit.Network.shared
                                .requestString("https://fapi.darock.top:65535/radar/reply/LWT/\(id)/\(enced)")
                            { respStr, isSuccess in
                                if isSuccess {
